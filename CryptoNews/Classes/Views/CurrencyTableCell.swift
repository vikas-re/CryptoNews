...
//  CryptoNews
//
//  Created by Vikas Sharma on 30/01/22.
//
...
//        let currencyIconPath = String(format: "%@%@/%@/%@", kCurrencyIconPath, kCurrencyIconColor, (currency.symbol ?? "").lowercased(), kCurrencyIconSize)
        ...
        imgCurrency.sd_setImage(with: currencyIconURL, placeholderImage: UIImage(named: "icLogo"), options: SDWebImageOptions(rawValue: 0), completed: { image, error, cacheType, imageURL in
