...
//  CryptoNews
//
//  Created by Vikas Sharma on 29/01/22.
//
...
class NewsTableCell: UITableViewCell {
    ...
    func configureNewsCell(news: RSSFeedItem) {
        if let thumbnail = news.media?.mediaContents?[0], let imagePath = thumbnail.attributes?.url {
            ...
            imgNews.sd_setImage(with: imageURL, placeholderImage: UIImage(named: "icLogo"), options: SDWebImageOptions(rawValue: 0), completed: { image, error, cacheType, imageURL in
